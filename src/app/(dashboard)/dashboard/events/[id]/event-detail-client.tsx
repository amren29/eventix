...
const TICKET_COLORS = ["#4f46e5", "#7c3aed", "#10b981", "#f59e0b", "#ef4444", "#06b6d4"];
...
function TabOverview({
  ...
  const capacityPct = event.total > 0 ? Math.round((event.sold / event.total) * 100) : 0;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label="Tickets Sold"
          value={`${event.sold} / ${event.total}`}
          trend={{ value: `${capacityPct}%`, direction: "up", label: "capacity" }}
          icon={Ticket}
        ...
        <StatCard
          label="Gross Revenue"
          value={formatCents(event.revenue)}
          icon={DollarSign}
        ...
        <StatCard
          label="Checked In"
          value={`${event.checkedIn} / ${event.sold}`}
          icon={CheckSquare}
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-neutral-100 shadow-sm p-5">
          ...
          <div className="space-y-4">
                ...
                <div key={t.id}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div>
                      ...
                      <p className="text-xs text-neutral-400">{formatCents(t.price)} · {t.sold} sold</p>
                    </div>
                    <p className="text-sm font-bold text-neutral-700">{formatCents(t.revenue)}</p>
                  </div>
            ...
            {ticketTypes.length === 0 && (
              <p className="text-sm text-neutral-400">No ticket types configured yet.</p>
            )}
          </div>
        </div>
...
function TabOrders({ orders }: { orders: OrderRow[] }) {
            ...
            {filtered.map((order) => (
              <tr key={order.id} className="hover:bg-neutral-50/50 transition-colors">
                <td className="px-5 py-3.5">
                  <code className="text-xs font-mono text-primary-600">{order.id.slice(0, 8)}</code>
                </td>
                <td className="px-5 py-3.5">
                ...
                <td className="px-5 py-3.5 text-right">
                  <p className="font-bold text-neutral-900 text-sm">{formatCents(order.amount)}</p>
                </td>
                <td className="px-5 py-3.5 text-center">
                  <Badge className={`text-[10px] px-2 h-5 border capitalize ${statusStyle[order.status] ?? "bg-neutral-100 text-neutral-500 border-neutral-200"}`}>
                    {order.status}
                  </Badge>
                </td>
...
function TabAttendees({ attendees }: { attendees: AttendeeRow[] }) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-4 flex items-center gap-4">
        <div className="flex-1">
          ...
          <Progress value={attendees.length > 0 ? (checkedIn / attendees.length) * 100 : 0} className="h-2 bg-neutral-100 [&>div]:bg-success-500" />
        </div>
        <div className="text-2xl font-extrabold text-success-600 flex-shrink-0">
          {attendees.length > 0 ? Math.round((checkedIn / attendees.length) * 100) : 0}%
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm overflow-hidden">
          ...
          <tbody className="divide-y divide-neutral-50">
            {attendees.map((attendee, i) => (
              <tr key={i} className="hover:bg-neutral-50/50 transition-colors">
                ...
                <td className="px-5 py-3.5 hidden lg:table-cell">
                  <code className="text-xs font-mono text-primary-600">{attendee.order.slice(0, 8)}</code>
                </td>
                <td className="px-5 py-3.5 text-center">
        ...
        {attendees.length === 0 && (
          <div className="text-center py-12 text-neutral-400 text-sm">No attendees yet.</div>
        )}
      </div>
    </div>
  );
...
function TabCheckIn({ event }: { event: EventData }) {
  const gates = [
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Checked In", value: String(event.checkedIn), sub: `of ${event.sold} attendees`, color: "text-neutral-900" },
          { label: "Check-In Rate", value: event.sold > 0 ? `${Math.round((event.checkedIn / event.sold) * 100)}%` : "0%", sub: event.checkedIn === 0 ? "event hasn't started" : "live", color: "text-neutral-900" },
...
function TabAnalytics({
  ...
  const avgOrderValue = paidOrders.length > 0 ? Math.round(grossRevenue / paidOrders.length) : 0;

  const chartData = ticketTypes.map((t, idx) => ({
    ...t,
    color: TICKET_COLORS[idx % TICKET_COLORS.length],
  }));
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Gross Revenue",   value: formatCents(grossRevenue), sub: "before fees" },
          { label: "Net Revenue",     value: formatCents(netRevenue),   sub: "after 2.5% fee" },
          { label: "Avg Order Value", value: formatCents(avgOrderValue), sub: "per transaction" },
          { label: "Total Orders",    value: String(paidOrders.length),  sub: "paid orders" },
        ].map(({ label, value, sub }) => (
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={chartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                <YAxis tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v / 100}`} />
                <Tooltip formatter={(v: any) => [formatCents(Number(v)), "Revenue"]} contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #e2e8f0" }} />
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-5">
        ...
        <div className="space-y-3">
          {promoCodes.map((code) => (
            <div key={code.id} className="flex items-center gap-4 p-3 rounded-xl bg-neutral-50 border border-neutral-100">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <div className="flex-1 max-w-32 h-1.5 bg-neutral-200 rounded-full overflow-hidden">
                    <div className="h-full gradient-primary rounded-full" style={{ width: `${code.limit ? (code.used / code.limit) * 100 : 50}%` }} />
                  </div>
                  <span className="text-xs text-neutral-400">
                    {code.used}{code.limit ? ` / ${code.limit}` : ""} uses
                  </span>
                </div>
              </div>
          ...
          {promoCodes.length === 0 && (
            <p className="text-sm text-neutral-400 py-4 text-center">No promo codes created yet.</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-5">
        ...
        <div className="bg-neutral-900 rounded-xl p-4 font-mono text-xs text-success-400 mb-3 overflow-x-auto">
          {`<script src="https://eventix.io/widget.js" data-event="${event.slug}"></script>`}
        </div>
...
function TabSettings({ event }: { event: EventData }) {
  return (
...
export function EventDetailClient({
  ...
  const tabContent: Record<string, React.ReactNode> = {
    overview:  <TabOverview event={event} ticketTypes={ticketTypes} salesTrend={salesTrend} />,
    orders:    <TabOrders orders={orders} />,
    attendees: <TabAttendees attendees={attendees} />,
    checkin:   <TabCheckIn event={event} />,
    analytics: <TabAnalytics event={event} ticketTypes={ticketTypes} orders={orders} />,
    marketing: <TabMarketing promoCodes={promoCodes} event={event} />,
    settings:  <TabSettings event={event} />,
  };
