...
export default function AttendeesClient({ attendees, events }: { attendees: Attendee[]; events: string[] }) {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Check-In Rate", value: attendees.length > 0 ? `${Math.round((checkedInCount / attendees.length) * 100)}%` : "0%", icon: QrCode, color: "text-accent-600", bg: "bg-accent-50" },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-4">
            ...
            {s.label === "Check-In Rate" && (
              <Progress value={attendees.length > 0 ? (checkedInCount / attendees.length) * 100 : 0} className="h-1 mt-2" />
            )}
          </div>
        ))}
      </div>
